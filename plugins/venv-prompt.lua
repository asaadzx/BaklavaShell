...
local function detect_env()
  local now = os.time()
  if now - cache_ts < CACHE_TTL and last_pwd ~= "" then return end
  ...
  if venv then
    ...
    if name then active_env = name; return end
  end

  local conda = os.getenv("CONDA_DEFAULT_ENV")
  if conda then active_env = conda; return end

  local pwd = last_pwd
  while pwd and #pwd > 0 do
    local handle = io.open(pwd .. "/.venv/pyvenv.cfg")
    if handle then handle:close(); active_env = pwd:match("/([^/]+)$") or "venv"; return end
