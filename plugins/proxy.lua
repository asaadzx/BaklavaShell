...
local function matches_pattern(host, pattern)
  ...
  if pattern:sub(1, 1) == "*" then
    return host:sub(-#pattern:sub(2)) == pattern:sub(2)
  end
  return host == pattern
end
...
local function find_proxy(host)
  for pat, proxy in pairs(patterns) do
    if matches_pattern(host, pat) then return proxy end
  end
  return nil
end
...
local function update_proxy()
  ...
  if proxy and not active then
    os.setenv("HTTP_PROXY", proxy); os.setenv("HTTPS_PROXY", proxy)
    os.setenv("http_proxy", proxy); os.setenv("https_proxy", proxy)
    active = true; current_proxy = proxy
...
function execute_command(args)
  ...
  if args[1] == "proxy" then
    if #args == 1 then
      if active then print("proxy: enabled (" .. current_proxy .. ")")
      else print("proxy: disabled") end
      return true
    end
    if args[2] == "on" then
    ...
    if args[2] == "off" then
      os.setenv("HTTP_PROXY", ""); os.setenv("HTTPS_PROXY", "")
      os.setenv("http_proxy", ""); os.setenv("https_proxy", "")
      active = false; print("proxy: disabled")
      return true
    end
