...
-- Shows the active Node.js version from .nvmrc or .node-version in the prompt
...
local function detect_node()
  local now = os.time()
  local pwd = io.popen("pwd"):read("*l") or ""
  if now - cache_ts < CACHE_TTL and pwd == last_pwd then return end
  last_pwd = pwd; cache_ts = now
